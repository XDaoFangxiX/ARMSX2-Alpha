...
fun PerformanceTab(state: MutableState<Settings>) {
    ...
    Column(
        ...
        run {
            ...
            val SuperLowEnd = Settings.lowEndPreset(
                s.copy(eeCycleRate = -5, eeCycleSkip = 1, mtvu = true, vu1Instant = true,
                    upscaleFloat = 0.5f, accurateBlendingUnit = 1, vuFlagHack = true, intcStat = true, waitLoop = true, fastCDVD = true, instantDma = true, BlitInternalFps = true),
                mtvu = com.armsx2.DeviceTier.mtvuDefault(),
            )
            // -1 = no preset matches (custom): no segment highlighted.
            val idx = when (s) { safe -> 0; fast -> 1; lowEnd -> 2; else -> -1; SuperLowEnd -> 3; else -> -5 }
            SegmentedRow(
                label = str("perf.speedhackProfile.label"),
                options = listOf(str("perf.speedhackProfile.optimal"), str("perf.speedhackProfile.fast"), str("perf.speedhackProfile.lowEnd"), str("perf.speedhackProfile.SuperLowEnd")),
                selectedIndex = idx,
                onChange = { when (it) { 0 -> apply(safe); 1 -> apply(fast); 2 -> apply(lowEnd); 3 -> apply(SuperLowEnd) } },
            )
        }
        ...
        CollapsibleSection(str("perf.speedhacks.title"), initiallyExpanded = true) {
            ...
            IntSliderRow(
                ...
                value = s.fpsLimit.coerceIn(0, 120),
                min = 0,
                max = 120,
            ...
            IntSliderRow(
                ...
                value = s.framerateNtsc.roundToInt().coerceIn(5, 120),
                min = 5,
                max = 120,
            ...
            IntSliderRow(
                ...
                value = s.frameratePal.roundToInt().coerceIn(5, 120),
                min = 5,
                max = 120,
